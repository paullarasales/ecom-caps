<x-manager-layout>
    ...
    <div class="absolute">
        <a href="{{route('manager.customfood.view')}}">
    ...
    <div class="text-center py-2 my-2">
        ...
        <h3 class="text-3xl sm:text-4xl leading-normal font-extrabold tracking-tight text-gray-900">
            Edit <span class="text-yellow-600">Food Item</span>
        </h3>

    </div>

    
    <form action="{{route('manager.customfood.update', $food->food_id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <script>
                ...
                <div class="bg-white rounded shadow-lg shadow-yellow-100 p-4 px-4 md:p-8 mb-6">
                    <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                            <div class="text-gray-600">
                                <p class="font-medium text-lg">Edit Food Item</p>
                        ...
                        <div class="lg:col-span-2">
                            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-4">
                                <div class="md:col-span-2">
                                    <label for="foodaname">Food Name</label>
                                    <input type="text" name="foodname" id="foodname" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50 focus:outline-none focus:border-yellow-500 focus:ring-yellow-500 focus:ring-1" value="{{$food->foodname}}" />
                                </div>
    
                                <div class="md:col-span-2">
                                    <label for="foodprice">Price</label>
                                    <input type="text" name="foodprice" id="foodprice" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50 focus:outline-none focus:border-yellow-500 focus:ring-yellow-500 focus:ring-1" value="{{$food->foodprice}}" />
                                </div>
