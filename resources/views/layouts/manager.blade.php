...
<body class="font-poppins antialiased">
    <div class="flex flex-col min-h-screen sm:flex-row">
        ...
        <aside class="bg-white text-black w-full sm:w-64 p-4 flex-shrink-0 sm:h-screen" x-data="{ open: false }">
            ...
            <nav :class="{ 'block': open, 'hidden': !open }" class="flex flex-col mt-10 gap-3 sm:block">
                <!-- Sidebar links with responsive adjustments -->
                <div class="{{ request()->routeIs('managerdashboard') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerdashboard') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('managerdashboard') }}" :active="request()->routeIs('managerdashboard')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Dashboard')}}
                    </x-side-nav-link>
                </div>
                ...
                <div class="{{ request()->routeIs('managerappointments') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerappointments') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                ...
                <div class="{{ request()->routeIs('managerpackages') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerpackages') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                    ...
                    <x-side-nav-link href="{{ route('managerpackages') }}" :active="request()->routeIs('managerpackages')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Packages')}}
                    </x-side-nav-link>
                </div>
        
                <div class="{{ request()->routeIs('managerreviewapproved') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerreviewapproved') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.562.562 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('managerreviewapproved') }}" :active="request()->routeIs('managerreviewapproved')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Reviews')}}
                    </x-side-nav-link>
                </div>

                <div class="{{ request()->routeIs('managerreports') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerreports') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25M9 16.5v.75m3-3v3M15 12v5.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('managerreports') }}" :active="request()->routeIs('managerreports')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Reports')}}
                    </x-side-nav-link>
                </div>
        
                <div class="{{ request()->routeIs('managerusers') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('managerusers') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z" clip-rule="evenodd" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('managerusers') }}" :active="request()->routeIs('managerusers')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Users')}}
                    </x-side-nav-link>
                </div>
            ...
            <nav class="bg-white shadow-sm">
                <div class="mx-auto px-2 sm:px-6 lg:px-8">
                    <div class="relative flex items-center justify-between h-16">
                        <div>
                            <h1 class="text-2xl font-medium">Manager</h1>
                        </div>
