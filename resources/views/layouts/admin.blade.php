    ...
    <div class="flex flex-col min-h-screen sm:flex-row">
        ...
        <aside class="bg-white text-black w-full sm:w-64 p-4 flex-shrink-0 sm:h-screen" x-data="{ open: false }">
            ...
            <nav :class="{ 'block': open, 'hidden': !open }" class="flex flex-col mt-10 gap-3 sm:block">
                ...
                <div class="{{ request()->routeIs('admindashboard') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('admindashboard') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('admindashboard') }}" :active="request()->routeIs('admindashboard')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Dashboard')}}
                    </x-side-nav-link>
                </div>
        
                <div class="{{ request()->routeIs('admin.chat') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12 relative">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('admin.chat') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                ...
                <div class="{{ request()->routeIs('adminappointments') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('adminappointments') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('adminappointments') }}" :active="request()->routeIs('adminappointments')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Events')}}
                    </x-side-nav-link>
                </div>
        
                <div class="{{ request()->routeIs('adminpackages') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('adminpackages') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                    ...
                    <x-side-nav-link href="{{ route('adminpackages') }}" :active="request()->routeIs('adminpackages')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Packages')}}
                    </x-side-nav-link>
                </div>
                ...
                <div class="{{ request()->routeIs('adminreports') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('adminreports') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25M9 16.5v.75m3-3v3M15 12v5.25m-4.5-15H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('adminreports') }}" :active="request()->routeIs('adminreports')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Reports')}}
                    </x-side-nav-link>
                </div>
        
                <div class="{{ request()->routeIs('adminusers') ? 'bg-gray-200' : '' }} flex items-center gap-2 rounded-sm h-12">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="{{ request()->routeIs('adminusers') ? '#FFB200' : '#000000'}}" class="ml-4 sm:ml-10 w-6 h-6">
                        <path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z" clip-rule="evenodd" />
                    </svg>
        
                    <x-side-nav-link href="{{ route('adminusers') }}" :active="request()->routeIs('adminusers')" class="text-xl text-black font-medium mt-1 flex items-center w-full">
                        {{ __('Users')}}
                    </x-side-nav-link>
                </div>
        ...
        <main class="flex-1 bg-gray-100 p-1">
            ...
            <nav class="bg-white shadow-sm">
                <div class="mx-auto px-2 sm:px-6 lg:px-8">
                    <div class="relative flex items-center justify-between h-16">
                        <div>
                            <h1 class="text-2xl font-medium">Admin</h1>
                        </div>
            ...
            <script>
                // Polling for unread appointment count
                setInterval(function() {
                    fetch('{{ route('fetch.admin.unopened.message.count') }}')
                        .then(response => response.json())
                        .then(data => {
