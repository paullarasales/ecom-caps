                                                        ...
                                                        @foreach ($items as $item)
                                                            <li class="flex justify-between">
                                                                ...
                                                                @if ($item->item_type === 'food_pack')
                                                                    (₱{{ $item->item_price ?? 'N/A' }} x {{ $item->quantity ?? 'N/A' }}pcs)
                ...
                <div class="flex justify-end gap-3 my-5">
                    ...
                    @if($appointment->edate && $appointment->etime)
                        @if(\Carbon\Carbon::parse($appointment->edate)->isPast())
                            {{-- <form id="deleteForm" action="{{ route('appointment.delete', $appointment->appointment_id) }}" method="POST">
                                @csrf
                                @method("DELETE")
                            ...
                            <a href="{{ route('appointment.archive.pending', $appointment->appointment_id ) }}" class="inline-flex items-center w-25 px-2 py-2 text-sm font-medium text-center text-white bg-yellow-700 rounded-lg hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800">
                                Archive
                        ...
                        <form id="acceptForm" action="{{ route('appointment.accept', $appointment->appointment_id) }}" method="POST" style="display: none;">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="status" value="{{ $appointment->status }}">
                    ...
                    @endif
                        ...
                        <a href="{{ route('pending.details.edit', $appointment->appointment_id) }}" class="inline-flex items-center w-25 px-2 py-2 text-sm font-medium text-center text-white bg-yellow-700 rounded-lg hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800">
                            Edit
                        ...
                        <form id="cancelForm" action="{{ route('appointment.cancel.meeting', $appointment->appointment_id) }}" method="POST" style="display: none;">
                            @csrf
                            @method("PUT")
                            <input type="hidden" name="status" value="{{ $appointment->status }}">
